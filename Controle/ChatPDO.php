...
class ChatPDO extends PDOBase
{
    ...
    function enviaMensagem()
    {
        ...
        $stmt = $pdo->prepare("insert into chat values (:id_chat , :id_remetente , :id_destinatario , 0 , '' , :mensagem , default , 0)");
        $stmt->bindValue(":id_chat", Uuid::uuid4()->toString());
...
//        }
        ...
        $pdo = conexao::getConexao();
        $stmt = $pdo->prepare("insert into chat values (:id_chat , :id_remetente , :id_destinatario , 1 , './Img/Chat/Media/" . $nome_imagem . $extFinal . "' , '' , default , 0)");
        $stmt->bindValue(":id_chat", Uuid::uuid4()->toString());
        $stmt->bindValue(":id_remetente", $this->logado->getId_usuario());
        $stmt->bindValue(":id_destinatario", $_GET['destinatario']);
        if ($stmt->execute()) {
